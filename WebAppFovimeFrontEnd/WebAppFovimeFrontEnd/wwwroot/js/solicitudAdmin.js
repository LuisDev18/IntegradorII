...
function GetSolicitudesAll() {    
    ...
    $('#tb_solicitudes').DataTable().destroy();

    datatable_solicitudes = $('#tb_solicitudes').DataTable({
        "ajax": {            
            "url": rootUrl + "/Simulator/GetSolicitudesAll",
            "type": "GET",
            "datatype": "json"
        },
        "columns": [
            ...
            },
            {
                ...
                "render": function (idSimulacion) {                    
                        return `<span class="text-center">                        
                                ...
                                &nbsp;
                                &nbsp;
                                <img onclick="GetSolicitudEstado(${idSimulacion})" src="${rootUrl}/img/edit-doc.png"  title="Actualizar estado de solicitud" style="cursor:pointer"/>
                            </span>`;                    
                }
            }
        ],
...
function GetSolicitudEstado(idSimulacion) {
    document.querySelector("#modal-status").showModal();

    $.ajax({
        method: "GET",
        dataType: "json",
        url: "/Simulator/GetSolicitudEstado?idSimulacion=" + idSimulacion,
        error: function () {
        ...
        success: function (response) {            
            if (response.ok == true) {                
                document.getElementById("selectEstado").value = response.idEstado;         
                document.getElementById("txtComentarios").value = response.comentarios;
            }
            else {
...
function CerrarModalEstatus() {
    document.querySelector("#modal-status").close();
}

function UpdateSolicitudEstado() {
    ...
    if (idEstado.length == 0) {
        ...
        $.ajax({
            method: "POST",
            dataType: "json",
            url: "/Simulator/UpdateSolicitudEstado",
            data: {
                'idEstado': idEstado,
    ...
    datatable_solicitudes_detail = $('#tb_solicitudes_detalle').DataTable({
        ...
        buttons: [
            ...
            },
            {
                ...
                filename: "Reporte de créditos",
                title: "<h6>Reporte detallado de créditos</h6>",
                exportOptions: {
                    columns: [0, 1, 2]
                },
